...
chevrotain.wet.fm1 <- jags(chevrotain.wet.data, chevrotain.wet.inits, params, 
                     here(models, "ssom_naive.txt"), 
                     n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb)
# combine all fitted models into list
ssom.fm1 <- list(pig.dry = wild.pig.dry.fm1, pig.wet = wild.pig.wet.fm1,
                 muntjac.dry = muntjac.dry.fm1, muntjac.wet = muntjac.wet.fm1,
                 chevrotain.dry = chevrotain.dry.fm1, 
                 chevrotain.wet = chevrotain.wet.fm1)

# check summary
ssom.fm1 |> lmap(print, dig = 3)

# list outputs to plot all models and parameters
ssom.fm1 |> 
  map_dfr(gather_draws, psi, p, .id = "model") |>
   separate_wider_delim(cols = model, 
                        delim = ".",
                        names = c("species", "season")) |> 
...
# Initial values
wild.pig.zobs <- apply(wild.pig2, c(1,3), function(x) max(x, na.rm = TRUE))
wild.pig.zobs[wild.pig.zobs == '-Inf'] <- NA     # site/years without visits
wild.pig.zst <- wild.pig.zobs     # Obs. occurrence as inits for z
wild.pig.inits <- function(){ list(z = wild.pig.zst)}
...
wild.pig.fm2 <- jags(wild.pig.data, wild.pig.inits, params, 
                     here(models,"dynocc_naive.txt"), 
...
# combine all fitted model outputs into list
dynocc.fm2 <- list(pig = wild.pig.fm2, muntjac = muntjac.fm2,
                    chevrotain = chevrotain.fm2)

# check summary
dynocc.fm2 |> lmap(print, dig = 3)

# set up summary tables for plots
# psi
dynocc.fm2 |> 
  map_dfr(gather_draws, psi[i], .id = "species") |> 
...
dynocc.fm2 |> 
  map_dfr(gather_draws, phi, eps, gamma, p, .id = "species") |> 
  ggplot(aes(x = .variable, y = .value, fill = species)) +
  stat_gradientinterval(position = "dodge", width = 0.5) +
  labs(title = "Wild pig, muntjac, chevrotain @ CCMNP 2023: DynOcc",
